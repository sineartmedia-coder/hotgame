...
const Game = ({ players, setPlayers, startingPlayer, onFinish, settings }) => {
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '1.2rem', zIndex: 10, marginTop: jokerNotice ? '50px' : '0', transition: 'margin 0.3s' }}>
        <motion.h2
                ...
                <div style={{ display: 'flex', gap: '12px', marginTop: '24px', flexDirection: 'column' }}>
                  ...
                  <div style={{ display: 'flex', gap: '12px' }}>
                    <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleAnswer(false)}
                      style={{ flex: 1, padding: '14px', background: '#d00000', color: 'white', border: 'none', borderRadius: '12px', fontWeight: '900', cursor: 'pointer' }}>❌ CEVAPLANMADİ</motion.button>
                    <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleAnswer(true)}
                      style={{ flex: 1, padding: '14px', background: '#2b9348', color: 'white', border: 'none', borderRadius: '12px', fontWeight: '900', cursor: 'pointer' }}>✅ CEVAPLANDI</motion.button>
                  </div>
                </div>
              )}
      ...
      <div style={{
        ...
        <MiniDeck count={players[currentPlayer].rejected} color="#d00000" icon="❌" label="Reddedilen" />
              ...
              return (
                <motion.div key={i}
                  ...
                  style={{
                    ...
                    fontSize: '1.7rem', cursor: isActive ? 'pointer' : 'default',
                ...
                >
                  🃏
                  {isActive && <span style={{ fontSize: '0.55rem', color: 'white', fontWeight: '900' }}>KULLAN</span>}
                </motion.div>
              );
            })}
          </div>
        ...
        <MiniDeck count={players[currentPlayer].completed} color="#2b9348" icon="✅" label="Tamamlanan" />
      </div>
